...
Shader::Shader()
	: m_FilePath(""), boulder_shaderID(0)
{
	std::cout << "Shader created id:" << boulder_shaderID << std::endl;
}

Shader::Shader(const std::string & filepath, bool useGeometryShader)
	: m_FilePath(filepath), boulder_shaderID(0)
{
	ShaderProgramSource source = ParseShader(filepath);

	if (useGeometryShader)
		boulder_shaderID = CreateShader(source.VertexSource, source.GeometrySource, source.FragmentSource);
	else
		boulder_shaderID = CreateShader(source.VertexSource, source.FragmentSource);
...
{
	GLCall(glUseProgram(boulder_shaderID));
}
...
int Shader::GetUniformLocation(const std::string & name)
{
	...
	GLCall(int location = glGetUniformLocation(boulder_shaderID, name.c_str()));
